---
name: preflight
description: 动手写代码前做一次对称决策预研 / Symmetric pre-flight research before writing code。钉死一句话需求 → 真实检索现成方案（GitHub、包注册表、社区踩坑帖）→ 实测基线并把「做」与「不做」两侧同口径摆出来 → 输出 复用/改造/自研/不做 四选一结论。当用户开新项目或新模块、问「有没有现成的库」「要不要自己造轮子」「这个功能值不值得加」「现在改还是先跑一段看」、做方案选型或可行性判断时使用；also use for build-vs-buy, tech stack selection, feasibility, "should I write this myself", dependency-adoption decisions。纯改 bug、修报错、纯重构、用户已指定依赖或已定好实现方式时不要用。
...
- **淘汰必须写明理由**：许可证不兼容（GPL/AGPL 对闭源产品有传染风险）、最后一次提交 > 2 年且无活跃 fork、职责与现有模块重叠（会造出两套真相）——命中即淘汰，别把判断留给用户猜。
- **六类断言不许用现在时陈述**：版本号、API 签名、默认值、维护状态、许可证、「某功能是否存在」。这些是变化最快、错了必然返工的类别，而模型对旧知识和新知识的置信感是一样的——它不会觉得自己过时。规则：没有本轮抓到的来源，就写成「据我知识（约 ____ 年前后），需核实」，并**同时给出验证方式或验证成本**。只标注不动手算违规；填不出锚点年份说明这条本就该查。
...
2. **侦察。** 按 `references/search-playbook.md` 执行。**先探测再选通道**：`command -v gh`（Windows 用 `where gh`）有结果就用 `gh search repos`，没有就走网络检索工具；shell 无外网时别浪费轮次试 `curl` / `git clone`。探测结果按机器有效，不要沿用记忆里的旧结论。
